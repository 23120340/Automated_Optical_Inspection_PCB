...
> Tài liệu liên quan: [Kế hoạch phân loại 6.1](ke_hoach_pretrain_6_1_classification.md) | [Kế hoạch số hóa PCB](ke_hoach_so_hoa_mach_pcb_aoi.md) | [Định dạng CAD](../thiet_ke/cad_formats.md)
...
| :--- | :--- | :--- | :--- |
| **1. Cấu trúc Tọa độ & Zero-Crop** | - Thiết lập dataclass `SolderPinDetection`, `ComponentDetectionRecord` trong `models.py`.<br>- Triển khai hàm quy đổi tọa độ `local_to_global_bbox()`. | `aoi_pipeline/models.py`<br>`aoi_pipeline/detection/cropping.py` | - Kiểm tra chuyển đổi tọa độ sai số $= 0.0\text{ px}$.<br>- Giải phóng 100% RAM ảnh crop sau inference. |
| **2. Tinh chỉnh Solder-Aware Padding** | - Bổ sung padding profile cho các nhóm footprint (0402, SOT-23, SOP, QFP).<br>- Giữ nguyên resolution gốc của ảnh canonial khi crop. | `aoi_pipeline/config.py`<br>`aoi_pipeline/detection/cropping.py` | - 100% crop chứa đủ chân hàn và pad kim loại.<br>- Không bị cắt cụt pad ở biên ảnh. |
| **3. Tích hợp Stage 2 Solder Detector** | - Xây dựng detector thứ cấp chuyên nhận diện chân hàn bên trong crop linh kiện.<br>- Kết hợp với mô hình phân loại lỗi (`solder defect classifier`). | `aoi_pipeline/solder/geometry.py`<br>`aoi_pipeline/pipeline.py` | - Tốc độ inference Stage 2 $\le 1.0\text{ ms}$ / linh kiện.<br>- Phát hiện chính xác từng chân pad riêng lẻ. |
