...
- `[x] XONG` **thêm / Claude:** `--leads truth|model|none` cho cổng (đối xứng `--families`). Nhánh `ic` của luật **chạy lần đầu tiên**; nó bị §8.2 chặn đúng thiết kế (thân aspect 1,13). Cổng cũng tách ba ca "2 cạnh" vốn in giống nhau, và tự reconfigure stdout sang UTF-8 thay vì dựa vào tác dụng phụ của việc nạp ONNX. 1150 test xanh.
- ⚠️ **Phát hiện lật lại một giả định đã ghi hai lần: nhánh `ic` bị đói vì NGUỒN CHÂN, không vì quy ước box cũ.** Đo bằng detector MỚI trên 4 nguồn ảnh + 12 tile: 81–92% chân pass-2 có tâm nằm TRONG box thân. Trên chính fixture đang có, cùng một bộ hộp thân: pad đếm tay nằm ngoài 24/28, chân pass-2 nằm trong 18/60. Nguyên nhân là `detect_leads_in_components` cắt cửa sổ QUANH linh kiện rồi tìm mối hàn bên trong. **Dựng fixture mới KHÔNG mở được nhánh `ic`** — chi tiết ở kế hoạch package §9.0c.
- ⚠️ **Giấy phép cho PK3–PK5:** `datasets/test_images/` bị `.gitignore` chặn và CVL PCB-DSLR là phi thương mại, tile phái sinh mang theo ràng buộc — **không commit được**. MPI-PCB gas pump là **CC BY 4.0**, commit được kèm attribution.
- `[x] XONG` **thêm / Claude 2026-09-06:** (a) tách được họ `capacitor` — aspect < 1,17, nghiệm thu **90,5%** vs baseline 68,2%, chia hiệu chỉnh/nghiệm thu theo bo; độ tròn §8.8 **đã đo và loại** vì nó chỉ ngược. (b) `_edge_of` đổi sang đo **mép ngoài** sau khi thử cả 5 phương án — chân rơi vào trong thân **18 → 0**, luật bỏ qua **15 → 3**. Cổng vẫn PASS cả 4 tổ hợp, 0 mất pad. 1155 test xanh. Chi tiết §6.3d, §6.3c-bis, §9.0e.
- ⚠️ **Xin lỗi, và cần các bạn biết:** commit `411c618` của tôi đã **quét luôn phần sửa kế hoạch chưa commit của các bạn** vào commit của tôi (bảng trạng thái §1 viết lại + tham chiếu §9.0d). Nội dung còn nguyên, không mất chữ nào, nhưng lý do sửa thì mất khỏi git log. Tôi dùng `git add -A` — lỗi của tôi. Từ giờ tôi chỉ add đúng file mình sửa. Không viết lại lịch sử vì nhánh đã push.
- ⚠️ **§9.0d đang là tham chiếu treo.** Các bạn dẫn tới nó ở hai chỗ (bảng §1 và mục nguồn nhãn) với nghĩa "cổng mới chỉ đo nhánh hình học trước fusion", nhưng mục đó chưa được viết. Tôi đã đánh mục mới của mình là **§9.0e** để chừa chỗ, không viết hộ.
- ⚠️ **ĐỔI TÊN THƯ MỤC TÀI LIỆU 2026-09-06 — đường dẫn cũ không còn.** Theo yêu cầu của người dùng (thống nhất một format, dùng tiếng Anh):

  | cũ | mới |
  |---|---|
  | `Docs/` | `docs/` |
  | `Docs/bao_cao/` | `docs/reports/` |
  | `Docs/danh_gia/` | `docs/evaluation/` |
  | `Docs/ke_hoach/` | `docs/plans/` |
  | `Docs/khao_sat/` | `docs/surveys/` |
  | `Docs/thiet_ke/` | `docs/design/` |

  Dùng `git mv` nên phần các bạn đang sửa dở trong `ke_hoach_phan_nhom_package.md` **giữ nguyên nội dung**, chỉ đổi đường dẫn thành `docs/plans/ke_hoach_phan_nhom_package.md`. **Tên file không đổi**, chỉ tên thư mục. 47 file đã được sửa tham chiếu; `tests/test_docs_links.py` xanh.
...
|---|---|---|
| Độ phân giải thật của ảnh | **~46 µm/px** | 3 phép đo độc lập, xem `docs/design/yeu_cau_phan_cung_camera.md` |
...
- `[x] HOÀN THÀNH` **C0a.** Khảo sát → `docs/surveys/dataset_lead_detection.md`.
  ~~**Kết luận: không nguồn công khai nào đủ dùng.**~~ **Kết luận này đã bị bác
  bỏ ngày 28/08** (`07bbe99`, banner ở đầu `docs/surveys/dataset_lead_detection.md`).
...
- `[x] HOÀN THÀNH` **D1.** Soạn yêu cầu tối thiểu về camera/ống kính/ánh sáng.
  → `docs/design/yeu_cau_phan_cung_camera.md`. Chốt: hiện 46 µm/px (3 phép đo độc lập,
...
- `[x] HOÀN THÀNH` **D3.** *(phát sinh)* Đánh giá camera có sẵn Hikvision
  DS-2CD5026WZ-YD 11–40 mm → mục 6 của `docs/design/yeu_cau_phan_cung_camera.md`.
...
`docs/evaluation/danh_gia_model_6_2.md`, chốt lại bằng `tests/inspection/test_solder_model_assessment.py`.
...
- `[x] ĐÃ HOÀN THÀNH` **J1.** Agent con khảo sát dataset + chọn model. Kết quả
  ...
  `docs/surveys/dataset_lead_detection.md`, mục "Cập nhật 2026-08-22".
...
| 2026-08-21 | D1 — yêu cầu phần cứng | `docs/design/yeu_cau_phan_cung_camera.md` |
...
| 2026-08-21 | C0a–C0e: khảo sát + notebook | `docs/surveys/dataset_lead_detection.md`, notebook lượt 2; 443/443 test pass |
...
| 2026-08-22 | I1 — đánh giá model 6.2 | `docs/evaluation/danh_gia_model_6_2.md`: chưa dùng để quyết định được, 61% mối hàn bị gọi `bridge` |
...
| 2026-08-24 | Kế hoạch fine-tune tại chỗ | `docs/plans/ke_hoach_fine_tune_cuc_bo.md`; đo trên máy này: khả thi ở imgsz 256, ~1–5 giờ |
...
| 2026-08-31 | Kế hoạch phân nhóm package | `docs/plans/ke_hoach_phan_nhom_package.md`; đo được 13,5% linh kiện mang 31,2% mối hàn và đều chỉ có chân ở 2/4 cạnh |
...
| 2026-08-31 | Kế hoạch lỗi toàn mạch | `docs/plans/ke_hoach_pcb_defect_toan_mach.md` — **chờ duyệt**; VisA pcb1–4 là nguồn có xước trên board ĐÃ LẮP, script fetch đã có sẵn |
