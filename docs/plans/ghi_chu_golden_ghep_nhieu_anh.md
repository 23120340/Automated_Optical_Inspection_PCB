...
- `docs/plans/ke_hoach_so_hoa_mach_pcb_aoi.md` — ảnh → CAD/netlist/recipe,
  **giả định đã có một ảnh Golden tốt**. Ghi chú này là tiền đề còn thiếu của
  nó.
- `docs/plans/ke_hoach_phan_nhom_package.md` §10.2 — chốt topology + ROI một
  lần lúc tạo golden recipe. Việc đó chỉ đúng nếu ảnh Golden đo được ở **mọi**
  vùng của bo.
- `docs/design/yeu_cau_phan_cung_camera.md` — nơi đặt phương án 3.
